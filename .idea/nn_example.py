...
possible_atom_list = [
    'C', 'N', 'O', 'S', 'F', 'Si', 'P', 'Cl', 'Br', 'Mg', 'Na', 'Ca', 'Fe', 'As', 'Al', 'I', 'B', 'V', 'K', 'Tl', 'Yb', 'Sb', 'Sn', 'Ag', 'Pd', 'Co', 'Se', 'Ti', 'Zn', 'H', 'Li', 'Ge', 'Cu', 'Au', 'Ni', 'Cd', 'In', 'Mn', 'Zr', 'Cr', 'Pt', 'Hg', 'Pb', 'W', 'Ru', 'Nb', 'Re', 'Te', 'Rh', 'Tc', 'Ba', 'Bi', 'Hf', 'Mo', 'U', 'Sm', 'Os', 'Ir', 'Ce','Gd','Ga','Cs', 'unknown'
]
...
def atom_features(atom,
  ...
  if bool_id_feat:
    ...
    results = one_of_k_encoding_unk(
      atom.GetSymbol(),
      ['C', 'N', 'O', 'S', 'F', 'Si', 'P', 'Cl', 'Br', 'Mg', 'Na', 'Ca', 'Fe', 'As', 'Al', 'I', 'B', 'V', 'K', 'Tl', 'Yb', 'Sb', 'Sn', 'Ag', 'Pd', 'Co', 'Se', 'Ti', 'Zn', 'H', 'Li', 'Ge', 'Cu', 'Au', 'Ni', 'Cd', 'In', 'Mn', 'Zr', 'Cr', 'Pt', 'Hg', 'Pb', 'W', 'Ru', 'Nb', 'Re', 'Te', 'Rh', 'Tc', 'Ba', 'Bi', 'Hf', 'Mo', 'U', 'Sm', 'Os', 'Ir', 'Ce','Gd','Ga','Cs', 'unknown']) + one_of_k_encoding(atom.GetDegree(),
...
def mol2vec(smiles):
  ...
  adj_matrix = np.pad(adj_matrix, [(0, (n_features - len(adj_matrix))), (0, (n_features - len(adj_matrix)))], mode='constant', constant_values=0)
  ...
  data = Data(x=torch.tensor(adj_matrix, dtype=torch.float, device=device),
              node_f=torch.tensor(node_f, dtype=torch.float, device=device),
...
def mol2vec_sol(smiles):
  ...
  for bond in bonds:
    edge_attr.append(bond_features(bond))
  data = torch.tensor(adj_matrix, dtype=torch.float, device=device)
  return data
...
def mol2vec_dual(smiles):
  ...
  adj_matrix1 = Chem.GetAdjacencyMatrix(mol1)

  mol2 = Chem.MolFromSmiles(smiles_two)
  adj_matrix2 = Chem.GetAdjacencyMatrix(mol2)

  adj_matrix = block_diag(adj_matrix1, adj_matrix2)
  adj_matrix = np.pad(adj_matrix, [(0, (n_features - len(adj_matrix))), (0, (n_features - len(adj_matrix)))], mode='constant', constant_values=0)
...
class Net(torch.nn.Module):
    def __init__(self):
        ...
        self.conv3 = GCNConv(16, n_features)
...
def train(epoch):
    ...
    for data in train_loader:
        ...
        optimizer.step()
    return float(loss_all) / len(train_dataset)
...
for epoch in range(1, end_range):
...
ax.plot([e for e in range(1,end_range)], hist["loss"], label="train_loss")
ax.plot([e for e in range(1,end_range)], hist["acc"], label="train_acc")
ax.plot([e for e in range(1,end_range)], hist["test_acc"], label="test_acc")
plt.xlabel("epoch")
ax.legend()