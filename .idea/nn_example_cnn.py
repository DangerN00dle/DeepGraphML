...
class Net(torch.nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(n_features, 16, 5 )
        self.pool = nn.MaxPool2d(4, 4)
        self.conv2 = nn.Conv2d(16, 32, 5)
        self.fc1 = nn.Linear(n_features*n_features, n_features)
        self.fc2 = nn.Linear(n_features, n_features)


    def forward(self, data):
        x, edge_index = data.x, data.edge_index
        x = self.pool(F.relu(self.conv1(x, edge_index)))
        x = self.pool(F.relu(self.conv2(x, edge_index)))
        x = x.view(-1, n_features*n_features)
        x = F.relu(self.fc1)
        x = F.relu(self.fc2)
